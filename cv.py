...
if __name__ == "__main__":    # Training settings
    ...
    for fold,(train_idx, test_idx) in enumerate(k_folds(n_splits = 10)):
        ...
        torch.utils.data.Subset(train_dataset,train_idx), batch_size=args.batch_size, shuffle=True, num_workers=1)
        val_loader = torch.utils.data.DataLoader(
        torch.utils.data.Subset(train_dataset,test_idx), batch_size=args.batch_size, shuffle=False, num_workers=1)
        ...
        for epoch in range(1, args.epochs + 1):
            train(epoch)
            validation()
            model_file = args.experiment + '/model_' + str(epoch) + '_fold_'+ str(fold) +'.pth'
